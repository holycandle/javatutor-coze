...
_MARKDOWN_RULES = """
## Markdown 格式约束（必须严格遵守）

格式化要求：
- 标题：「###」与标题文字之间必须有空格，如「### 核心结论」。标题前后必须有空行。
- 分隔线：「---」前后必须各有一个空行独立成行，不能紧贴文字。
- 代码块：围栏「```java」之后、「```」之前必须换行，代码内代码必须逐行换行，不能挤成一行。
- 列表：「- 项目」的「-」与文字之间必须有空格，列表项之间换行。
- 段落：每个段落之间用空行分隔，不要把所有内容写成一个超长段落。
- 行内：用「**加粗**」、「`代码`」时正常使用即可。
