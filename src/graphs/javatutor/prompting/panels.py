...
def render_run_mode_guide() -> str:
    """生成「运行模式判读」引导段——诊断启发式，**不进本体**。

    切分理由（review P3-1 订正过）：本体 ``user_guides`` 只经 ``render_usage_guide()`` 进**主 Agent**
    系统提示（Judge 的地基 ``build_judge_grounding_block()`` 只含 ``field_schema`` + ``modules``）；
    把「判错会误诊」这类启发式混进本体，等于把诊断口径塞进领域知识。故知识进本体、规则留在此处。
    事实（这次是哪种模式）由 ``context_builder`` 的 ``### 运行模式`` packet 给出——packet 说值，本段说规则。

    **推论（勿再弄反）**：Judge 侧唯一能看到运行模式事实的通道是 ``contexts.build_facts_block``
    的那一行，去掉它评审就会把「当前是默认模式」判成幻觉。
    """
    return (
        "## 运行模式判读（判错会直接误诊）\n\n"
        "JavaTutor 有两种运行模式，**只有在事实来源里写明的那种模式才成立**——模式事实有两个来源，"
        "**以系统注入的 `### 运行模式` 上下文为准**（它与本次请求同时取值）；"
        "用户提问里可能另有一个 `[运行环境]` 块（报错入口预填，写于点击那一刻，"
        "用户若在发送前改了用例，它可能已过期）。**两者都没有**才算模式未知：\n\n"
        ...
        "规则：\n"
        "- 模式事实写着**默认模式（测试模式未激活）**时，**不得**据此断言代码有语法/逻辑错误；\n"
        "  应先说明「当前以默认模式执行，测试模式未激活」，并指出激活办法（在「测试」面板输入用例后点「保存」）。\n"
        "- 模式事实写着**测试模式**时，不要建议用户「补一个 main」。\n"
        "- 模式事实**两个来源都没有**（旧客户端）时，**不要臆测模式**；按错误原文本身判断，必要时问用户当时用的是哪种模式。"
    )
