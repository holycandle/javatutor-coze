...
| 联调修复计划（概念题被当「当前步」作答 + 优化第二步不出代码） | 已执行（Task 1–4 / 6–7 / 9–10 离线全绿：coze 447（本件完成时）/ 前端 445；与下述评审优化合并后 coze **484**；**Task 5 / Task 8 已按 §6 不再单独执行**——其内容由下述评审优化 spec 的 CD-4 取代、D6 收编进 Plan A Task 3，避免两份计划并行改评审表。**Bug D 根因定位为判别器冲突**（第二步提问形状 = 「已指明目标」形状，同形输入要求走两条分支）⇒ 判别器改为显式 `【优化第二步】` 标记，由前端写进提问、两侧字面量硬编码。**Task 0 现场证据仍待联调侧**；线上未验证。**2026-09-14 追加审查判决：Bug D 未修好**——联调反馈「仍重复给优化选项」，已在本已部署构建上端到端复现，性质为**间歇性**（同一提问 10 次 1 次 options / 9 次 replace），故标记判别器是**概率性缓解而非门闩**；见本文件 `docs/reviews/2026-09-14-optimization-step2-marker-nondeterminism-review.md`） |
| 优化第二步判别器非确定性 review | 已审查（**用户反馈成立 / 1 项待改验收口径**）：线上复现「带 `【优化第二步】` 的提问仍拿到 `options` 卡」，排除「未重发」（痕迹批次三键 + `git log -S` 同提交 `12e3f4b`）与「意图是触发器」（ablation 2 次均 replace），定性为**间歇**（10 次 1 options / 9 replace，成功时 `replace` 块形态全对）⇒ 根因 = 全链路无环节检查「带标记的提问是否拿到 replace」，唯一约束是系统提示条款 + 一条与本例几乎逐字相同的 few-shot；**建议复用 propose→guard 回路加确定性门闩**（判据纯 JSON：恰好一个 `【编辑建议】` 块且 `kind=="replace"`、`code` 非空无 `...` 占位），验收由「1 次通过」改「N=10 全 replace」+ FakeModel 门闩用例。附：`intent` 被误判 `data_query`（关键词含「变量」，命中选项文案）属**真缺陷但非本次触发条件**，且该耦合是本批 Task 3 新开的；探针第三副本字面量漏标记 + `parse_options` 脆解析已修）。**判决已由确定性门闩收口（离线），线上待重发**——见下两行 |
| 优化第二步交付形态门闩 plan | 已执行（Task 1–8 离线全绿：coze **511 passed**（基线 484，+27）/ 前端 445 / `npm run build` ok / L2.5 5 passed / L3 `ok` / L5 外壳两条命令均无输出；L4 SKIP）。新增纯函数门闩 `harness/answer_gate.py`（判据 `check_step2_answer`：恰好一个【编辑建议】块且 `kind=="replace"`、`code` 非空无 `...` 占位），图节点 `answer_gate` 11→15，违规回灌 `HumanMessage` 并**与 `guard` 共享轮次预算**（`g+r<=3`），痕迹两键 `optimize_step2_gate` / `optimize_step2_retries`。三处偏差：① `verdict` 取词落 `"passed"`（与 `guard_decision` 词汇同字）、用例落成模块级函数；② `EDIT_MARKER` 沿用既有两处本地副本惯例 + 守漂移断言（harness 不 import 节点层）；③ 既有终止性算术的**朴素上界 25 在加入门闩后不可达**，测试改为联合界 `13+3g+2r`（最大 22）——收紧而非放宽。**Task 8 的线上 N=10 验收须先重发 agent，未做——线上未验证** |
| 评审-修订子系统优化 spec | 已定稿（联调反馈「评审很鸡肋，总把较正确的答案改得答非所问」；**量化取证**：四轮归档 124 样本中评审拦截 33 条，其中 **11 条 Judge 判 `correct`（误杀 33%）**，`incorrect` 只拦下 10/22（召回 45%），修订请求时延 +48%；6 条决策 CD-1…CD-6：回滚闸 / 最小编辑 / 意见必须给出处 / 评审表分流 + 补「正面回答」条 / advisory 开关 / 可观测性） |
...
| 优化第二步判别器非确定性 review | `docs/reviews/2026-09-14-optimization-step2-marker-nondeterminism-review.md` | 上行的**事后审查**：Bug D 仍未修好（线上复现），性质为间歇（10 次 1 options/9 replace）；推荐确定性门闩而非强化提示词；含探针工具两处缺陷的修复（漏标记的字面量第三副本、`parse_options` 脆解析静默回落 fixture） |
| 优化第二步交付形态门闩计划 | `docs/plan/2026-09-14-optimization-step2-delivery-gate-plan.md` | 上述审查的 TDD 实施计划（Task 1–8）：判据纯函数 `check_step2_answer` → 节点 `answer_gate` → 共享 `MAX_ROUNDS` 预算 → 痕迹两键 → 意图守卫 → 文档；含**联合界**改法（`13+3g+2r`，最大 22）与「单次通过不构成证据」的验收口径修正 |
| 优化第二步交付形态门闩实现 | `docs/devlog/2026-09-14-optimization-step2-delivery-gate.md` | 门闩落地记录：纯判据（平衡 JSON 解析 + varargs 例外）+ 回灌走 `HumanMessage`（Bug A 红线，被拒终答绝不写 `AIMessage`）+ 与 `guard` 共享预算（收束轮后不再重试）+ 意图守卫；coze **511**（基线 484）/ 前端 445 / L2.5 / L3 / L5 全绿，L4 SKIP。**§4.1 两处偏差**（`EDIT_MARKER` 沿用节点层副本惯例、朴素上界 25 不可达故改联合界）、**§4.2 三条遗留**（线上 N=10 未验 / 门闩只判形态不判语义 / 接口 spec trace 示例的既有文档债未顺手补） |
